...
func (dag *Dag) executeSimple(progress *Progress, simpleNode *SimpleNode, r *CompositionRequest) (bool, error) {
	...
	// uncomment to debug test TestInvokeFC_Concurrent
	// Debug(r, string(simpleNode.Id), output)
...
func Debug(r *CompositionRequest, nodeId string, output map[string]interface{}) (bool, error) {
	if strings.Contains(r.ReqId, "goroutine") {
		...
		if expectedOutput != actualOutput {
			...
			return false, fmt.Errorf("expected: %d - actual: %v\n", expectedOutput, output)
		}
	}
	return true, nil
}
